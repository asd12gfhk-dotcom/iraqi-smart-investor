...
export interface ConfirmedMajorMove {
  ...
  calendarDays?: number;
  maxPriceInLeg?: number;
  minPriceInLeg?: number;
}
...
export interface HistoricalEpisode {
  ...
  troughDate: string;
  endDate: string;
  drawdownPct: number; // e.g. -18.5 (%)
  declineDurationBars: number;
  recoveryPct: number; // e.g. +22.4 (%)
  recoveryDurationBars: number;
  peakPrice: number;
  troughPrice: number;
}

export interface CandidateCycle {
  hasValidCycle: boolean;
  statusLabel: string;
  medianPeriodBars: number;
  meanPeriodBars: number;
  stdDevBars: number;
  percentileP25: number;
  percentileP75: number;
  occurrencesCount: number;
  successfulReversalsCount: number;
  failuresCount: number;
  medianDrawdownPct: number;
  medianRecoveryPct: number;
  currentBarsSinceTrough: number;
  cycleProgressPct: number;
  isInTroughWindow: boolean;
}
...
export interface HistoricalSimilarityMatch {
  ...
  post5dReturnPct: number;
  post10dReturnPct: number;
  post20dReturnPct: number;
  post30dReturnPct: number;
  maxDrawdownPostSignalPct: number;
}

export interface HistoricalSimilarityEngineResult {
  matchedCount: number;
  winRate20dPct: number; // e.g. 75 (%)
  medianPost20dReturnPct: number;
  minPost20dReturnPct: number;
  maxPost20dReturnPct: number;
  targetReturnPct: number;
  matches: HistoricalSimilarityMatch[];
  summaryText: string;
}

export interface LiquidityInflowResult {
  caseType: 'CASE_A_PANIC' | 'CASE_B_ACCUMULATION' | 'CASE_C_REBOUND' | 'CASE_D_WEAK_BOUNCE' | 'NEUTRAL';
  caseTitle: string;
  volumeRatio5d: number;
  valueRatio5d: number;
  tradesRatio5d: number;
  accumulationTrend: string;
  isPositiveInflow: boolean;
  description: string;
}

export interface SellingExhaustionResult {
  isExhausted: boolean;
  priceDecelerationScore: number; // 0 to 100
  isPriceFloorEstablished: boolean;
  holdingBarsCount: number;
  volumeDivergence: boolean;
  rsiOversoldTurningUp: boolean;
  score: number; // 0 to 100
  description: string;
}

export interface StockMonthlySeasonality {
...
export interface StockSeasonalityResult {
  currentMonth: StockMonthlySeasonality;
  bestMonths: StockMonthlySeasonality[];
  worstMonths: StockMonthlySeasonality[];
  allMonths: StockMonthlySeasonality[];
  seasonalityLabel: string;
}

export interface BacktestValidationResult {
  sampleSignalsCount: number;
  inSampleWinRatePct: number;
  outOfSampleWinRatePct: number;
  medianReturnPct: number;
  maxDrawdownPct: number;
  isUnstableModel: boolean; // UNSTABLE_MODEL
  stabilityStatus: 'نموذج مستقر' | 'نموذج غير مستقر (UNSTABLE_MODEL)';
}

export interface EvidenceGateStatus {
  gate1DataQuality: { passed: boolean; message: string; details: string };
  gate2HistoricalPattern: { passed: boolean; message: string; details: string };
  gate3CurrentState: { passed: boolean; message: string; details: string };
  gate4LiquidityReversal: { passed: boolean; message: string; details: string };
  gate5Validation: { passed: boolean; message: string; details: string };
  allGatesPassed: boolean;
}

export type HistoricalSignalType = 'BUY' | 'BUY_WATCH' | 'WATCH' | 'EXIT' | 'HOLD' | 'NO_SIGNAL';

export interface HistoricalDecisionResult {
  ticker: string;
  companyNameAr: string;
  asOfDate: string;
  currentPrice: number;
  signal: HistoricalSignalType;
  signalBadgeAr: string;
  signalColor: string; // 'emerald' | 'amber' | 'rose' | 'slate'
  confidenceScore: number; // 0 to 100
  signalId: string; // Deterministic signal ID for deduplication
  reasons: string[];
  cancellationConditions: string[];
  episodes: HistoricalEpisode[];
  cycle: CandidateCycle;
  similarity: HistoricalSimilarityEngineResult;
  liquidity: LiquidityInflowResult;
  exhaustion: SellingExhaustionResult;
  seasonality: StockSeasonalityResult;
  backtest: BacktestValidationResult;
  gates: EvidenceGateStatus;
