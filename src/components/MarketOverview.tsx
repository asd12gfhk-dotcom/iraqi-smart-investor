        ...
        <div className="lg:col-span-2 bg-white border border-zinc-200 rounded-2xl p-5 shadow-sm space-y-4">
          <div className="flex items-center justify-between">
            <h3 className="text-sm font-bold text-zinc-900 flex items-center gap-2">
              ...
              أداء القطاعات الفني والسيولة الموجهة
            </h3>
            <span className="text-xs text-zinc-500">متوسط التغير السعري وسيولة القطاع</span>
          </div>

          <div className="h-64">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={sectorChartData} margin={{ top: 10, right: 10, left: 10, bottom: 20 }}>
                <XAxis dataKey="sector" stroke="#a1a1aa" tick={{ fill: '#52525b', fontSize: 11 }} />
                <YAxis stroke="#a1a1aa" tick={{ fill: '#52525b', fontSize: 11 }} />
                <Tooltip
                  contentStyle={{ backgroundColor: '#ffffff', borderColor: '#e4e4e7', borderRadius: '12px', color: '#18181b', boxShadow: '0 4px 6px -1px rgba(0, 0, 0, 0.1)' }}
                  formatter={(val: any, name: any) => [
                    name === 'avgChangePct' ? `${val}%` : `${val}M د.ع`,
                    name === 'avgChangePct' ? 'متوسط التغير' : 'قيمة التداول'
                  ]}
                />
                <Bar dataKey="avgChangePct" name="avgChangePct" fill="#d97706" radius={[4, 4, 0, 0]} />
              </BarChart>
            </ResponsiveContainer>
          </div>
        </div>

        {/* Market Advance / Decline Breadth Ratio */}
        <div className="bg-white border border-zinc-200 rounded-2xl p-5 shadow-sm space-y-4 flex flex-col justify-between">
          <div>
            <h3 className="text-sm font-bold text-zinc-900 mb-3">اتساع السوق (Advancers vs Decliners)</h3>
            <div className="space-y-3">
              <div className="flex items-center justify-between text-xs font-bold">
                <span className="text-emerald-700 flex items-center gap-1">
                  <ArrowUpRight className="w-4 h-4" /> مرتفعة ({summary.advancersCount})
                </span>
                <span className="text-rose-700 flex items-center gap-1">
                  منخفضة ({summary.declinersCount}) <ArrowDownRight className="w-4 h-4" />
                </span>
              </div>
              ...
              <div className="h-3 bg-zinc-200 rounded-full overflow-hidden flex">
                <div
                  ...
                  style={{ width: `${(summary.advancersCount / summary.tradedCompaniesCount) * 100}%` }}
                ></div>
                <div
                  className="bg-zinc-400 h-full transition-all"
                  style={{ width: `${(summary.unchangedCount / summary.tradedCompaniesCount) * 100}%` }}
                ></div>
                <div
                  className="bg-rose-500 h-full transition-all"
                  style={{ width: `${(summary.declinersCount / summary.tradedCompaniesCount) * 100}%` }}
                ></div>
              </div>

              <div className="flex justify-between text-[11px] text-zinc-500 pt-1">
                <span>ارتفاع: {((summary.advancersCount / summary.tradedCompaniesCount) * 100).toFixed(0)}%</span>
                <span>ثبات: {summary.unchangedCount} شركة</span>
                <span>تراجع: {((summary.declinersCount / summary.tradedCompaniesCount) * 100).toFixed(0)}%</span>
              </div>
            </div>
          </div>

          <div className="p-3 bg-amber-50 rounded-xl border border-amber-200 text-xs text-zinc-800 leading-relaxed">
            <strong className="text-amber-800 block mb-1">الرأي الفني العام للسوق:</strong>
            يتسم أداء جلسة اليوم بسيطرة المشترين وتفوق الشركات المرتفعة مع ارتفاع أحجام التداول في الأسهم القيادية، مما يشير إلى استمرار النمط الصاعد لمؤشر ISX60.
          </div>
        </div>
      </div>
