...
import { computeCoreV22Systems } from '../utils/evaluatorEngine';
import {
  getTelegramConfig,
...
export const TechnicalAnalysisModule: React.FC<TechnicalAnalysisModuleProps> = ({
  ...
  const handleOpenAlertModal = () => {
    ...
    window.history.pushState({ modalOpen: 'techAlertModal' }, '');
  };
  ...
  const recentMajorMoves = evaluation?.recentMajorMoves || [];
  ...
  return (
    <div className="space-y-6">
      ...
      {/* ========================================================================= */}
      {/* MALEQ PROFESSIONAL (CORE V2.2) ADVANCED TECHNICAL SYSTEMS */}
      {/* ========================================================================= */}
      {(() => {
        const coreV22 = evaluation.coreV22 || computeCoreV22Systems(
          history,
          company.currentPrice,
          indicators,
          scoreBreakdown,
        ...
        return (
          <div className="bg-zinc-950 text-white border border-amber-500/30 rounded-2xl p-6 shadow-xl space-y-6">
            <div className="flex flex-col md:flex-row md:items-center justify-between pb-4 border-b border-zinc-800 gap-3">
              <div className="flex items-center gap-3 text-amber-400">
                <span className="w-8 h-8 rounded-xl bg-amber-500 text-zinc-950 flex items-center justify-center font-black font-mono text-sm shadow-md">
                  V2.2
                </span>
                <div>
                  <h3 className="text-base font-bold text-white flex items-center gap-2">
                    الملحق الاحترافي (Core V2.2) — الأنظمة المتقدمة للتحليل الفني
                  </h3>
                  <p className="text-xs text-zinc-400">
                    نظام حتمي 100% قابل للحساب والتحقق رياضياً بدون أخبار أو تقدير بشري
                  </p>
                </div>
              </div>
              <div className="flex items-center gap-2 font-mono text-xs">
                <span className="px-3 py-1 bg-amber-500/10 border border-amber-500/30 text-amber-400 rounded-lg font-bold">
                  النتيجة المركبة النهائي: {coreV22.finalScoreboard.compositeScore.value}/100 ({coreV22.finalScoreboard.compositeScore.starsStr})
                </span>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 text-xs">
              {/* System 18: Money Rotation */}
              <div className="p-4 bg-zinc-900/90 border border-zinc-800 rounded-xl space-y-2">
                <div className="flex items-center justify-between">
                  <span className="font-bold text-amber-400">18. دوران السيولة (Rotation)</span>
                  <span className={`px-2 py-0.5 rounded text-[11px] font-bold ${
                    coreV22.moneyRotation.classification === 'دخول سيولة' ? 'bg-emerald-500/20 text-emerald-400 border border-emerald-500/30' :
                    coreV22.moneyRotation.classification === 'خروج سيولة' ? 'bg-rose-500/20 text-rose-400 border border-rose-500/30' :
                    coreV22.moneyRotation.classification === 'تدوير سيولة' ? 'bg-blue-500/20 text-blue-400 border border-blue-500/30' :
                    'bg-zinc-800 text-zinc-300'
                  }`}>
                    {coreV22.moneyRotation.classification}
                  </span>
                </div>
                <div className="text-[11px] text-zinc-300 space-y-1 font-mono">
                  <div className="flex justify-between"><span>حصة السهم الحالية:</span><strong>{coreV22.moneyRotation.stockShareCurrentPct}%</strong></div>
                  <div className="flex justify-between"><span>حصة السهم السابقة:</span><strong>{coreV22.moneyRotation.stockSharePrevPct}%</strong></div>
                  <div className="flex justify-between"><span>الفارق:</span><strong className={coreV22.moneyRotation.shareDiffPct >= 0 ? 'text-emerald-400' : 'text-rose-400'}>{coreV22.moneyRotation.shareDiffPct}%</strong></div>
                  <div className="flex justify-between"><span>مسار القطاع:</span><strong>{coreV22.moneyRotation.sectorTrend}</strong></div>
                </div>
              </div>

              {/* System 19: Volatility Engine */}
              <div className="p-4 bg-zinc-900/90 border border-zinc-800 rounded-xl space-y-2">
                <div className="flex items-center justify-between">
                  <span className="font-bold text-amber-400">19. محرك التقلب (Volatility)</span>
                  <span className={`px-2 py-0.5 rounded text-[11px] font-bold ${
                    coreV22.volatility.classification === 'مرتفع جداً' ? 'bg-rose-500/20 text-rose-400 border border-rose-500/30' :
                    coreV22.volatility.classification === 'مرتفع' ? 'bg-amber-500/20 text-amber-400 border border-amber-500/30' :
                    'bg-emerald-500/20 text-emerald-400 border border-emerald-500/30'
                  }`}>
                    {coreV22.volatility.classification}
                  </span>
                </div>
                <div className="text-[11px] text-zinc-300 space-y-1 font-mono">
                  <div className="flex justify-between"><span>ATR يومي:</span><strong>{coreV22.volatility.dailyAtrPct}%</strong></div>
                  <div className="flex justify-between"><span>ATR أسبوعي:</span><strong>{coreV22.volatility.weeklyAtrPct}%</strong></div>
                  <div className="flex justify-between"><span>ATR شهري:</span><strong>{coreV22.volatility.monthlyAtrPct}%</strong></div>
                  <div className="flex justify-between"><span>متوسط التقلب:</span><strong className="text-amber-300">{coreV22.volatility.avgAtrPct}%</strong></div>
                </div>
              </div>

              {/* System 20: Breakout Engine */}
              <div className="p-4 bg-zinc-900/90 border border-zinc-800 rounded-xl space-y-2">
                <div className="flex items-center justify-between">
                  <span className="font-bold text-amber-400">20. محرك الاختراقات (Breakout)</span>
                  <span className="text-[11px] font-mono text-zinc-400">نسبة الحجم: {coreV22.breakout.volumeRatio.toFixed(2)}x</span>
                </div>
                <div className="text-[11px] text-zinc-300 space-y-1">
                  <div className="flex justify-between"><span>نوع الحركة:</span><strong className="font-mono text-amber-300">{coreV22.breakout.type}</strong></div>
                  <div className="flex justify-between"><span>إغلاق متجاوز:</span><strong>{coreV22.breakout.isClosedBeyond ? 'نعم ✓' : 'لا ✗'}</strong></div>
                  <div className="pt-1 border-t border-zinc-800 font-bold text-[11px] text-amber-400 text-center">
                    {coreV22.breakout.qualification}
                  </div>
                </div>
              </div>

              {/* System 21: False Breakout Engine */}
              <div className="p-4 bg-zinc-900/90 border border-zinc-800 rounded-xl space-y-2">
                <div className="flex items-center justify-between">
                  <span className="font-bold text-amber-400">21. كاشف الاختراق الكاذب</span>
                  <span className="text-[11px] font-mono text-zinc-400">{coreV22.falseBreakout.sessionsObserved} جلسات مراقبة</span>
                </div>
                <div className="text-[11px] text-zinc-300 space-y-1">
                  <div className="flex justify-between"><span>حالة الاختراق:</span>
                    <strong className={
                      coreV22.falseBreakout.status === 'اختراق حقيقي' ? 'text-emerald-400' :
                      coreV22.falseBreakout.status === 'اختراق كاذب' ? 'text-rose-400' : 'text-amber-400'
                    }>
                      {coreV22.falseBreakout.status}
                    </strong>
                  </div>
                  <div className="text-[10px] text-zinc-400 pt-1">
                    فحص الثبات فوق / تحت المستويات المفصلية لـ 3 جلسات متتالية.
                  </div>
                </div>
              </div>

              {/* System 22: Accumulation & Distribution */}
              <div className="p-4 bg-zinc-900/90 border border-zinc-800 rounded-xl space-y-2">
                <div className="flex items-center justify-between">
                  <span className="font-bold text-amber-400">22. التجميع والتصريف (Accum/Dist)</span>
                  <span className={`px-2 py-0.5 rounded text-[11px] font-bold ${
                    coreV22.accumulationDistribution.classification === 'تجميع' ? 'bg-emerald-500/20 text-emerald-400 border border-emerald-500/30' :
                    coreV22.accumulationDistribution.classification === 'تصريف' ? 'bg-rose-500/20 text-rose-400 border border-rose-500/30' :
                    'bg-zinc-800 text-zinc-300'
                  }`}>
                    {coreV22.accumulationDistribution.classification}
                  </span>
                </div>
                <div className="text-[11px] text-zinc-300 space-y-1 font-mono">
                  <div className="flex justify-between"><span>درجة الثقة:</span><strong>{coreV22.accumulationDistribution.confidencePct}%</strong></div>
                  <div className="flex justify-between"><span>مصادر إيجابية:</span><strong>{coreV22.accumulationDistribution.positiveSourcesCount} من {coreV22.accumulationDistribution.totalSourcesCount}</strong></div>
                  <div className="flex justify-between"><span>مسار OBV:</span><strong>{coreV22.accumulationDistribution.obvTrend}</strong></div>
                </div>
              </div>

              {/* System 23: Trend Strength */}
              <div className="p-4 bg-zinc-900/90 border border-zinc-800 rounded-xl space-y-2">
                <div className="flex items-center justify-between">
                  <span className="font-bold text-amber-400">23. قوة الاتجاه (Trend Strength)</span>
                  <span className="text-amber-400 font-mono text-sm">{coreV22.trendStrength.starsStr}</span>
                </div>
                <div className="text-[11px] text-zinc-300 space-y-1 font-mono">
                  <div className="flex justify-between"><span>النقاط الرقمية:</span><strong className="text-amber-300">{coreV22.trendStrength.score}/100</strong></div>
                  <div className="flex justify-between"><span>النجوم الرسمية:</span><strong>{coreV22.trendStrength.stars} / 5 نجوم</strong></div>
                </div>
              </div>

              {/* System 24: Momentum Acceleration */}
              <div className="p-4 bg-zinc-900/90 border border-zinc-800 rounded-xl space-y-2">
                <div className="flex items-center justify-between">
                  <span className="font-bold text-amber-400">24. تسارع الزخم (Momentum)</span>
                  <span className="px-2 py-0.5 rounded text-[11px] font-bold bg-blue-500/20 text-blue-400 border border-blue-500/30">
                    {coreV22.momentumEngine.status}
                  </span>
                </div>
                <div className="text-[11px] text-zinc-300 space-y-1 font-mono">
                  <div className="flex justify-between"><span>درجة الزخم:</span><strong>{coreV22.momentumEngine.score}/100</strong></div>
                  <div className="flex justify-between"><span>تقاطع RSI 50:</span><strong>{coreV22.momentumEngine.rsiCrossed50 ? 'نعم ✓' : 'لا ✗'}</strong></div>
                </div>
              </div>

              {/* System 25: Volume Trend Agreement */}
              <div className="p-4 bg-zinc-900/90 border border-zinc-800 rounded-xl space-y-2">
                <div className="flex items-center justify-between">
                  <span className="font-bold text-amber-400">25. توافق الحجم مع الاتجاه</span>
                  <span className={`px-2 py-0.5 rounded text-[11px] font-bold ${
                    coreV22.volumeAgreement.status === 'يؤكد الاتجاه' ? 'bg-emerald-500/20 text-emerald-400 border border-emerald-500/30' :
                    coreV22.volumeAgreement.status === 'يخالف الاتجاه' ? 'bg-rose-500/20 text-rose-400 border border-rose-500/30' :
                    'bg-zinc-800 text-zinc-300'
                  }`}>
                    {coreV22.volumeAgreement.status}
                  </span>
                </div>
                <div className="text-[11px] text-zinc-300 space-y-1 font-mono">
                  <div className="flex justify-between"><span>درجة الحجم:</span><strong>{coreV22.volumeAgreement.volumeScore}/100</strong></div>
                </div>
              </div>

              {/* System 26: Liquidity Power */}
              <div className="p-4 bg-zinc-900/90 border border-zinc-800 rounded-xl space-y-2">
                <div className="flex items-center justify-between">
                  <span className="font-bold text-amber-400">26. قوة السيولة (Liquidity)</span>
                  <span className="text-amber-400 font-mono text-sm">{coreV22.liquidityPower.starsStr}</span>
                </div>
                <div className="text-[11px] text-zinc-300 space-y-1 font-mono">
                  <div className="flex justify-between"><span>درجة السيولة:</span><strong className="text-emerald-400">{coreV22.liquidityPower.score}/100</strong></div>
                  <div className="flex justify-between"><span>التقييم بالنجوم:</span><strong>{coreV22.liquidityPower.stars} نجوم</strong></div>
                </div>
              </div>

              {/* System 27: Price Quality Score */}
              <div className="p-4 bg-zinc-900/90 border border-zinc-800 rounded-xl space-y-2">
                <div className="flex items-center justify-between">
                  <span className="font-bold text-amber-400">27. جودة الحركة (Price Quality)</span>
                  <span className="text-amber-400 font-mono text-sm">{coreV22.priceQuality.starsStr}</span>
                </div>
                <div className="text-[11px] text-zinc-300 space-y-1 font-mono">
                  <div className="flex justify-between"><span>مؤشر الجودة:</span><strong className="text-amber-300">{coreV22.priceQuality.score}/100</strong></div>
                  <div className="flex justify-between"><span>الانعكاسات الحادة:</span><strong>{coreV22.priceQuality.reversalsCount}</strong></div>
                  <div className="flex justify-between"><span>اتساق المسار:</span><strong>{coreV22.priceQuality.trendConsistencyPct}%</strong></div>
                </div>
              </div>

              {/* System 28: Trend Health Score */}
              <div className="p-4 bg-zinc-900/90 border border-zinc-800 rounded-xl space-y-2">
                <div className="flex items-center justify-between">
                  <span className="font-bold text-amber-400">28. صحة الاتجاه (Trend Health)</span>
                  <span className="text-amber-400 font-mono text-sm">{coreV22.trendHealth.starsStr}</span>
                </div>
                <div className="text-[11px] text-zinc-300 space-y-1 font-mono">
                  <div className="flex justify-between"><span>درجة الصحة:</span><strong className="text-emerald-400">{coreV22.trendHealth.score}/100</strong></div>
                  <div className="flex justify-between"><span>الشروط المحققة:</span><strong>{coreV22.trendHealth.metConditionsCount} من {coreV22.trendHealth.totalConditionsCount}</strong></div>
                </div>
              </div>

              {/* System 29: Foreign Activity Score */}
              {coreV22.foreignActivity && (
                <div className="p-4 bg-zinc-900/90 border border-zinc-800 rounded-xl space-y-2">
                  <div className="flex items-center justify-between">
                    <span className="font-bold text-amber-400">29. نشاط غير العراقيين</span>
                    <span className="text-amber-400 font-mono text-sm">{coreV22.foreignActivity.starsStr}</span>
                  </div>
                  <div className="text-[11px] text-zinc-300 space-y-1 font-mono">
                    <div className="flex justify-between"><span>درجة النشاط:</span><strong className="text-teal-400">{coreV22.foreignActivity.score}/100</strong></div>
                    <div className="flex justify-between"><span>المشاركة:</span><strong>{coreV22.foreignActivity.participationScore}/100</strong></div>
                    <div className="flex justify-between"><span>الارتباط:</span><strong>{coreV22.foreignActivity.correlationScore}/100</strong></div>
                  </div>
                </div>
              )}
            </div>

            {/* System 30: Final Scoreboard Table */}
            <div className="p-4 bg-zinc-900 border border-zinc-800 rounded-xl space-y-3">
              <div className="flex items-center justify-between border-b border-zinc-800 pb-2">
                <span className="font-bold text-amber-400 text-sm">30. لوحة النتائج النهائية ونظام النجوم الموحد (Final Scoreboard)</span>
                <span className="text-xs text-zinc-400 font-mono">معيار النجوم: 90+=5★ | 75+=4★ | 60+=3★ | 40+=2★</span>
              </div>
              <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-3 text-xs font-mono">
                {Object.entries(coreV22.finalScoreboard).map(([key, val]) => {
                  const scoreItem = val as { value: number; stars: number; starsStr: string };
                  const labels: Record<string, string> = {
                    trendScore: 'الاتجاه العام',
                    momentumScore: 'الزخم النسبي',
                    volumeScore: 'حجم التداول',
                    liquidityScore: 'قوة السيولة',
                    patternScore: 'النماذج الفنية',
                    foreignScore: 'تدفق الأجانب',
                    confidenceScore: 'درجة الثقة',
                    compositeScore: 'التقييم المركب',
                    dataQualityScore: 'جودة البيانات',
                    signalStrengthScore: 'قوة الإشارة'
                  };

                  return (
                    <div key={key} className="p-2.5 bg-zinc-950 border border-zinc-800 rounded-lg text-center space-y-1">
                      <span className="text-[11px] text-zinc-400 block">{labels[key] || key}</span>
                      <strong className="text-amber-400 text-base block font-black">{scoreItem.value}/100</strong>
                      <span className="text-amber-300 text-xs block">{scoreItem.starsStr}</span>
                    </div>
                  );
                })}
              </div>
      ...
      {/* ========================================================================= */}
      {/* 6. التقرير المختصر (Executive / Short Report) */}
      {/* ========================================================================= */}
      <div className="bg-amber-50/70 border border-amber-200 rounded-2xl p-5 shadow-sm space-y-3">
        <div className="flex items-center justify-between pb-2 border-b border-amber-200">
          <div className="flex items-center gap-2 text-xs font-bold text-amber-900">
            <span className="w-5 h-5 rounded-full bg-amber-500 text-zinc-950 flex items-center justify-center font-mono">6</span>
            <span className="text-sm font-bold">التقرير التنفيذي المختصر (Short Executive Report)</span>
          </div>
          <span className={`px-3 py-1 rounded-full text-xs font-black ${evaluation?.badgeBg || 'bg-zinc-100'} ${evaluation?.badgeTextColor || 'text-zinc-800'}`}>
            القرار الرقمي: {evaluation?.tier || '-'} ({evaluation?.compositeScore ?? 0}/100)
          </span>
        </div>

        <p className="text-xs text-amber-950 leading-relaxed font-sans bg-white p-3 rounded-xl border border-amber-200/80">
          سهم <strong>{company?.nameAr} ({company?.ticker})</strong> عند سعر <strong>{(company?.currentPrice ?? 0).toFixed(2)} د.ع</strong> يحقق تقييماً فنياً مركباً بدرجة <strong>{evaluation?.compositeScore ?? 0}/100 ({evaluation?.tier || '-'})</strong> مع اتجاه متوسط المدى <strong>{trendAnalysis.mediumTerm.level}</strong> ومستوى ثقة <strong>{evaluation?.confidenceScore || '-'}</strong>. أظهر السهم حركة تجميع لغير العراقيين بحجم صافي <strong>{((nonIraqi?.netValue ?? 0) / 1000000).toFixed(2)}M د.ع</strong> مع مستويات دعم رئيسية عند <strong>{supports[0]?.price != null ? supports[0].price.toFixed(2) : 'غير محدد'} د.ع</strong> ومقاومة عند <strong>{resistances[0]?.price != null ? resistances[0].price.toFixed(2) : 'غير محدد'} د.ع</strong>.
        </p>
      </div>
      ...
      {/* ========================================================================= */}
      {/* 8. الحجم (Volume Breakdown) */}
      {/* ========================================================================= */}
      <div className="bg-white border border-zinc-200 rounded-2xl p-5 shadow-sm space-y-4">
        <div className="flex items-center justify-between pb-3 border-b border-zinc-200">
          <div className="flex items-center gap-2 text-xs font-bold text-amber-800">
            <span className="w-5 h-5 rounded-full bg-amber-500 text-zinc-950 flex items-center justify-center font-mono">8</span>
            <span className="text-sm text-zinc-900">تحليل أحجام التداول والتدفقات (Volume & Flow Analytics)</span>
          </div>
          <span className="text-xs font-mono font-bold text-purple-700">مقياس الحجم: {scoreBreakdown.volumeScore}/100</span>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-3 gap-3 text-xs">
          <div className="p-3 bg-zinc-50 rounded-xl border border-zinc-200 space-y-1">
            <span className="text-zinc-500 block">نسبة الحجم مقارنة بالمتوسط (20 يوم)</span>
            <span className="text-lg font-mono font-black text-purple-800 block">
              {((indicators?.volumeRatio20 ?? 0) * 100).toFixed(0)}%
            </span>
            <span className="text-[10px] text-zinc-600">
              {(indicators?.volumeRatio20 ?? 0) > 1.2 ? 'نشاط حجمي استثنائي متصاعد' : 'حجم تداول ضمن النطاق الطبيعي'}
            </span>
          </div>

          <div className="p-3 bg-zinc-50 rounded-xl border border-zinc-200 space-y-1">
            <span className="text-zinc-500 block">مؤشر الحجم التراكمي On-Balance Volume (OBV)</span>
            <span className="text-lg font-mono font-black text-zinc-900 block font-mono">
              {((indicators?.obv ?? 0) / 1000000).toFixed(2)}M
            </span>
            <span className="text-[10px] text-emerald-700 font-semibold">تأكيد اتجاه الأسعار بالاحجام</span>
          </div>

          <div className="p-3 bg-zinc-50 rounded-xl border border-zinc-200 space-y-1">
            <span className="text-zinc-500 block">مؤشر التدفق المالي Money Flow Index (MFI)</span>
            <span className="text-lg font-mono font-black text-amber-700 block">
              {(indicators?.mfi14 ?? 0).toFixed(1)}
            </span>
            <span className="text-[10px] text-zinc-600">سيولة مالية تدفقية (MFI 14)</span>
          </div>
        </div>
      </div>

      {/* ========================================================================= */}
      {/* 9. السيولة (Liquidity Engine) */}
      {/* ========================================================================= */}
      <div className="bg-white border border-zinc-200 rounded-2xl p-5 shadow-sm space-y-4">
        <div className="flex items-center justify-between pb-3 border-b border-zinc-200">
          <div className="flex items-center gap-2 text-xs font-bold text-amber-800">
            <span className="w-5 h-5 rounded-full bg-amber-500 text-zinc-950 flex items-center justify-center font-mono">9</span>
            <span className="text-sm text-zinc-900">محرك قياس السيولة والاستمرارية (Liquidity Engine)</span>
          </div>
          <span className="text-xs font-mono font-bold text-emerald-700">نقاط السيولة: {scoreBreakdown.liquidityScore}/100</span>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-3 gap-3 text-xs">
          <div className="p-3 bg-zinc-50 rounded-xl border border-zinc-200 space-y-1">
            <span className="text-zinc-500 block">الجلسات الفعلية ذات التداول</span>
            <span className="text-base font-mono font-bold text-zinc-900 block">
              {indicators?.activeBarsCount ?? 0} جلسة من أصل 60
            </span>
            <span className="text-[10px] text-zinc-600">نسبة التواجد الفعلي بالسوق</span>
          </div>

          <div className="p-3 bg-zinc-50 rounded-xl border border-zinc-200 space-y-1">
            <span className="text-zinc-500 block">متوسط عدد الصفقات اليومي</span>
            <span className="text-base font-mono font-bold text-zinc-900 block">
              {(history.reduce((a, b) => a + (b.trades || 1), 0) / Math.max(1, history.length)).toFixed(1)} صفقة/جلسة
            </span>
            <span className="text-[10px] text-zinc-600">كثافة التنفيذ الفعلي</span>
          </div>

          <div className="p-3 bg-zinc-50 rounded-xl border border-zinc-200 space-y-1">
            <span className="text-zinc-500 block">متوسط قيمة التداول اليومية</span>
            <span className="text-base font-mono font-bold text-amber-700 block">
              {(history.reduce((a, b) => a + (b.value || b.volume * b.close), 0) / Math.max(1, history.length) / 1000000).toFixed(2)}M د.ع
            </span>
            <span className="text-[10px] text-zinc-600">قدرة الاستيعاب للسيولة</span>
          </div>
        </div>
      </div>
      ...
      {/* ========================================================================= */}
      {/* 11. التفاصيل (Full Report Details & Weights) */}
      {/* ========================================================================= */}
      <div className="bg-white border border-zinc-200 rounded-2xl p-5 shadow-sm space-y-4">
        <div className="flex items-center justify-between pb-3 border-b border-zinc-200">
          <div className="flex items-center gap-2 text-xs font-bold text-amber-800">
            <span className="w-5 h-5 rounded-full bg-amber-500 text-zinc-950 flex items-center justify-center font-mono">11</span>
            <span className="text-sm text-zinc-900">التفاصيل الفنية الكاملة والأوزان المستعملة (Full Technical Details)</span>
          </div>
          <span className="text-xs text-zinc-500">نظام حتمي 100% بدون ذكاء افتراضي توليدي</span>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-xs">
          <div className="p-3.5 bg-zinc-50 rounded-xl border border-zinc-200 space-y-2">
            <h4 className="font-bold text-zinc-900 flex items-center gap-1">
              <FileText className="w-4 h-4 text-amber-600" /> أوزان المعادلة المركبة (Composite Formula Weights):
            </h4>
            <div className="space-y-1 font-mono text-[11px] text-zinc-700">
              <div className="flex justify-between border-b pb-1"><span>اتجاه المتوسطات (Trend):</span><strong>{(weightsUsed.trend * 100).toFixed(0)}%</strong></div>
              <div className="flex justify-between border-b pb-1"><span>مؤشرات الزخم (Momentum):</span><strong>{(weightsUsed.momentum * 100).toFixed(0)}%</strong></div>
              <div className="flex justify-between border-b pb-1"><span>نشاط الحجم (Volume):</span><strong>{(weightsUsed.volume * 100).toFixed(0)}%</strong></div>
              <div className="flex justify-between border-b pb-1"><span>استمرارية السيولة (Liquidity):</span><strong>{(weightsUsed.liquidity * 100).toFixed(0)}%</strong></div>
              <div className="flex justify-between border-b pb-1"><span>تداولات غير العراقيين (Foreign):</span><strong>{(weightsUsed.foreign * 100).toFixed(0)}%</strong></div>
              <div className="flex justify-between"><span>النماذج الفنية (Pattern):</span><strong>{(weightsUsed.pattern * 100).toFixed(0)}%</strong></div>
            </div>
          </div>

          <div className="p-3.5 bg-zinc-50 rounded-xl border border-zinc-200 space-y-2">
            <h4 className="font-bold text-zinc-900 flex items-center gap-1">
              <Activity className="w-4 h-4 text-amber-600" /> آخر التحركات الحادة المكتشفة (ZigZag 8% Threshold):
            </h4>
            {recentMajorMoves.length === 0 ? (
              <p className="text-zinc-500 text-[11px] py-2">لا توجد موجات تغير حاد تتجاوز 8% في الآونة الأخيرة.</p>
            ) : (
              <div className="space-y-2 text-xs">
                {recentMajorMoves.slice(0, 5).map((move, idx) => {
                  ...
                  return (
                    <div key={idx} className="p-2.5 bg-white rounded-xl border border-zinc-200 shadow-xs flex flex-col sm:flex-row sm:items-center justify-between gap-2">
                      <div className="flex flex-wrap items-center gap-2">
                        <span className={`px-2 py-0.5 rounded-md font-bold text-[11px] ${isUp ? 'bg-emerald-100 text-emerald-800 border border-emerald-200' : 'bg-rose-100 text-rose-800 border border-rose-200'}`}>
                          {isUp ? 'موجة صاعدة 📈' : 'موجة هابطة 📉'}
                        </span>
                        
                        <div className="flex items-center gap-1.5 font-mono text-[11px] text-zinc-700 bg-zinc-100 px-2 py-0.5 rounded-md">
                          <span>من <strong className="text-zinc-900">{startP.toFixed(2)}</strong></span>
                          <span>←</span>
                          <span>إلى <strong className="text-zinc-900">{endP.toFixed(2)} د.ع</strong></span>
                        </div>

                        <span dir="ltr" className="text-zinc-500 font-mono text-[11px] bg-zinc-50 px-1.5 py-0.5 rounded border border-zinc-200">
                          {move.startDate} → {move.endDate}
                        </span>

                        {days > 0 && (
                          <span className="text-[10px] text-zinc-400">({days} جلسة)</span>
                        )}
                      </div>

                      <div className="flex items-center gap-2 self-end sm:self-center font-mono">
                        <span className={`text-[11px] ${diffIQD >= 0 ? 'text-emerald-600' : 'text-rose-600'}`}>
                          ({diffIQD >= 0 ? '+' : ''}{diffIQD.toFixed(2)} د.ع)
                        </span>
                        <strong className={`font-bold text-sm px-2 py-0.5 rounded-md ${pct >= 0 ? 'bg-emerald-50 text-emerald-700 border border-emerald-200' : 'bg-rose-50 text-rose-700 border border-rose-200'}`}>
                          {pct >= 0 ? '+' : ''}{pct.toFixed(2)}%
                        </strong>
                      </div>
                    </div>
