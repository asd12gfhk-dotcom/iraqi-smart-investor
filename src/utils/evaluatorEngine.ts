...
export function calculateMajorMoves(bars: DailyBar[]): {
  ...
  const uniqueBarsMap = new Map<string, DailyBar>();
  for (const b of bars) {
    if (b && b.date && b.close > 0) {
      uniqueBarsMap.set(b.date, b);
    }
  }

  const activeBars = Array.from(uniqueBarsMap.values()).sort((a, b) => a.date.localeCompare(b.date));
  ...
  // Prepare structured rows matching computeZigZag
  const rows = activeBars.map((b, idx) => ({
    date: b.date,
    price: b.close,
    bar: b,
    sessionIndex: idx
  ...
  const pivots: { index: number; price: number; date: string; type: 'HIGH' | 'LOW' | null }[] = [];

  let lastPivotPrice = rows[0].price;
  let direction = 0; // 0 = unassigned, 1 = bullish, -1 = bearish
  let extremeIndex = 0;
  let extremePrice = rows[0].price;
  let minIndex = 0;
  let minPrice = rows[0].price;
  let maxIndex = 0;
  let maxPrice = rows[0].price;

  pivots.push({ index: 0, price: rows[0].price, date: rows[0].date, type: null });

  for (let i = 1; i < rows.length; i++) {
    const price = rows[i].price;

    if (direction === 0) {
      const changeFromStart = (price - lastPivotPrice) / lastPivotPrice;
      if (changeFromStart >= threshold) {
        pivots[0] = { index: minIndex, price: minPrice, date: rows[minIndex].date, type: 'LOW' };
        direction = 1;
        extremeIndex = i;
        extremePrice = price;
      } else if (changeFromStart <= -threshold) {
        pivots[0] = { index: maxIndex, price: maxPrice, date: rows[maxIndex].date, type: 'HIGH' };
        direction = -1;
        extremeIndex = i;
        extremePrice = price;
      } else {
    ...
    if (direction === 1) {
      if (price > extremePrice) {
        ...
        const dropFromExtreme = (price - extremePrice) / extremePrice;
        if (dropFromExtreme <= -threshold) {
          pivots.push({ index: extremeIndex, price: extremePrice, date: rows[extremeIndex].date, type: 'HIGH' });
          direction = -1;
          extremeIndex = i;
          extremePrice = price;
        }
      ...
      if (price < extremePrice) {
        ...
        const riseFromExtreme = (price - extremePrice) / extremePrice;
        if (riseFromExtreme >= threshold) {
          pivots.push({ index: extremeIndex, price: extremePrice, date: rows[extremeIndex].date, type: 'LOW' });
          direction = 1;
          extremeIndex = i;
          extremePrice = price;
        }
  ...
  // Push final extreme point reached
  const lastPivotInList = pivots[pivots.length - 1];
  if (!lastPivotInList || lastPivotInList.index !== extremeIndex) {
    pivots.push({
      index: extremeIndex,
      price: extremePrice,
      date: rows[extremeIndex].date,
      type: direction === 1 ? 'HIGH' : direction === -1 ? 'LOW' : null
    });
  }

  // Handle latest available session endpoint
  const lastIndex = rows.length - 1;
  const currentLastPivot = pivots[pivots.length - 1];
  if (lastIndex > currentLastPivot.index) {
    pivots.push({
      index: lastIndex,
      price: rows[lastIndex].price,
      date: rows[lastIndex].date,
      type: rows[lastIndex].price >= currentLastPivot.price ? 'HIGH' : 'LOW'
    });
  }

  // Deduplicate consecutive pivots with identical index
  const cleanPivots: typeof pivots = [];
  for (const p of pivots) {
    if (cleanPivots.length === 0 || cleanPivots[cleanPivots.length - 1].index !== p.index) {
      cleanPivots.push(p);
    }
  }

  // Construct swings matching Google Apps Script computeZigZag output
  const moves: ConfirmedMajorMove[] = [];
  for (let p = 1; p < cleanPivots.length; p++) {
    const prev = cleanPivots[p - 1];
    const curr = cleanPivots[p];

    if (prev.index === curr.index) continue;

    const fromRow = rows[prev.index];
    const toRow = rows[curr.index];

    const diffIQD = toRow.price - fromRow.price;
    const changePct = fromRow.price > 0 ? (diffIQD / fromRow.price) * 100 : 0;
    const type: 'صعود' | 'هبوط' = changePct >= 0 ? 'صعود' : 'هبوط';

    const d1 = new Date(fromRow.date).getTime();
    const d2 = new Date(toRow.date).getTime();
    const calendarDays = !isNaN(d1) && !isNaN(d2) ? Math.round(Math.abs(d2 - d1) / (1000 * 60 * 60 * 24)) : 0;
    const activeTradingDays = Math.abs(curr.index - prev.index);

    const legBars = activeBars.slice(Math.min(prev.index, curr.index), Math.max(prev.index, curr.index) + 1);
    const maxPriceInLeg = Math.max(...legBars.map((b) => (b.high > 0 ? b.high : b.close)));
    const minPriceInLeg = Math.min(...legBars.map((b) => (b.low > 0 ? b.low : b.close)));

    moves.push({
      type,
      startDate: fromRow.date,
      startPrice: fromRow.price,
      endDate: toRow.date,
      endPrice: toRow.price,
      changePct: parseFloat(changePct.toFixed(2)),
      activeTradingDays,
      calendarDays,
      maxPriceInLeg: parseFloat(maxPriceInLeg.toFixed(2)),
      minPriceInLeg: parseFloat(minPriceInLeg.toFixed(2))
    });
  }
  ...
  return {
    moves: sortedMoves,
    stats: {
      avgBullishPct: parseFloat(avgBullishPct.toFixed(2)),
      avgBearishPct: parseFloat(avgBearishPct.toFixed(2)),
      avgBearishActiveDays: parseFloat(avgBearishActiveDays.toFixed(1))
    }
  };
}
