 ...
 */
export function loadIsxData(raw) {
  const out = {};
...
export function buildFeatures(rows) {
  ...
  for (let i = 0; i < rows.length; i++) {
    ...
    f.ema50 = ema50[i];
    f.rsi14 = rsi14[i];
    f.volRatio20 = volSma20[i] ? v[i] / volSma20[i] : null;
    ...
    // RSI bands
    f.rsi30to50 = rsi14[i] !== null && rsi14[i] >= 30 && rsi14[i] <= 50;
    f.rsi40to55 = rsi14[i] !== null && rsi14[i] >= 40 && rsi14[i] <= 55;
    ...
    if (ema20[i] !== null && ema50[i] !== null && ema20[i] !== 0) {
      f.pullbackEma20 = Math.abs(cl / ema20[i] - 1) <= 0.02 && ema20[i] >= ema50[i];
    } else {
 ...
 */
export function detectWaves(rows, threshold = CONFIG.DEFAULT_THRESHOLD) {
  const n = rows.length;
...
export function medianGapDays(waves) {
 ...
 */
export function cycleTimingFlags(rows, waves) {
  const n = rows.length;
...
export function engineACycleHammer(rows, feats, ctFlags, i) {
  // Standalone: 27.7% precision, 1.75x lift (n=643)
  return ctFlags[i] && feats[i].hammer;
}

export function engineBCycleRsi40to55(rows, feats, ctFlags, i) {
  // Standalone: 27.5% precision, 1.74x lift (n=5531)
  return ctFlags[i] && feats[i].rsi40to55;
}

export function engineCCycleVolume3x(rows, feats, ctFlags, i) {
  // Standalone: 26.9% precision, 1.71x lift (n=828)
  return ctFlags[i] && feats[i].volume3x;
}

export function engineDCycleAnyOfFour(rows, feats, ctFlags, i) {
...
export function engineECyclePullbackEma20(rows, feats, ctFlags, i) {
  // Standalone: 25.7% precision, 1.63x lift (n=1505)
  return ctFlags[i] && feats[i].pullbackEma20;
}

export function engineFCycleRsi30to50(rows, feats, ctFlags, i) {
 ...
 */
export function analyzeSymbol(rows, threshold = CONFIG.DEFAULT_THRESHOLD, votesRequired = CONFIG.VOTES_REQUIRED) {
...
function eventNearFlags(rows, waves, leadDays = null) {
  const n = rows.length;
 ...
 */
export function runBacktest(bySymbol, threshold = CONFIG.DEFAULT_THRESHOLD, votesRequired = CONFIG.VOTES_REQUIRED) {
 ...
 */
export function toAlertRecord(result) {
 ...
 * a Telegram loop).
 */
export function runIsxEngine(rawIsxJson, options = {}) {
  ...
  const alerts = Object.values(bt.perSymbolResults)
    .filter((r) => r.combinedAlertToday && (r.lastVolume === undefined || r.lastVolume >= minVolume))
    .sort((a, b) => b.votesCountToday - a.votesCountToday)
    .map((r) => ({ ...toAlertRecord(r), message: formatAlert(r) }));
